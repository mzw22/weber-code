...
models <- models %>% select(!c("model"))
# Combine the study info and the results
k_results <- left_join(study_list, models, by="filename")

# Add columns for labels etc.
k_results <- k_results %>%
  mutate(species = ifelse(species_2 == "spp.", species_1,
                          paste(species_1, species_2, sep=" "))) %>%
  mutate(initial = ifelse(species_2 == "spp.", species_1,
                          paste0(substr(species_1, 1, 1), "."))) %>% #First initial
  mutate(species_short = paste(initial, species_2), # Short species name for labels
         species_full = paste(species_1, species_2)) %>% # Long species name for labels
