...
# Import raw data
input_data <- read.csv("../../data/musall2014/Figure1f_extracted.csv")

# Standardise data
data <- input_data %>%
  mutate(a = target_freq,
         b = norm_distractor*target_freq,  #norm distractor = distractor/target
         response=prop_discrimination,
         N = n_trials) %>%
...
write.csv(data, "../../results/standard_data/musall2014_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     ref="Musall et al. 2014",
                     file_prefix="musall2014",
                     n_pairs=n_pairs)
