...
# Import data
input_data <- read.csv("../../data/hanke2012/hanke2012_fig4a-c.csv")

# Subset: food experiment
data <- input_data %>%
  mutate(a = standard, b = paddle,
         response = percent_correct/100,
         N=80) %>% #each point = 80 trials
...
write.csv(data, "../../results/standard_data/hanke2012_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     family=binomial(link="logit"),
                     response_lab="Proportion correct",
                     stimulus="Paddle width (cm)",
                     ref="Hanke et al. 2012",
                     file_prefix="hanke2012",
                     n_pairs=n_pairs)
