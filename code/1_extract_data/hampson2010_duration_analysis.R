...
# Import data
input_data <- read.csv("../../data/hampson2010/temporal.csv")

# Any other changes go here
data <- input_data %>%
  mutate(B_correct = time_from_onset > reference) %>%
  mutate(prop_B = percent_b/100) %>%
  mutate(prop_correct = ifelse(B_correct, prop_B, 1-prop_B))

# Standardise data
data <- data %>%
  mutate(a = time_from_onset,
         b = reference,
         response = prop_correct,
         N = 40) %>% #n trials in a session
...
write.csv(data, "../../results/standard_data/hampson2010_duration_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="duration",
                     ref="Hampson et al. 2010 (duration)",
                     file_prefix="hampson2010_duration",
                     n_pairs=n_pairs)
