...
data <- input_data %>%
  filter(Stimuli == "Pebbles", Species == "SM") %>%
...
write.csv(data, "../../results/standard_data/gazes2017_pebbles_SM_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     ref="Gazes et al. 2017 (pebbles, spider monkey)",
                     file_prefix="gazes2017_pebbles_SM",
                     n_pairs=n_pairs)
