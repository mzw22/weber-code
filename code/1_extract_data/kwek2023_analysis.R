...
# Import data
input_data <- read.csv("../../data/kwek2023/published_data/FMC - Contrast.csv")

# Standardise data
data <- input_data %>%
  mutate(a = Male1contrast,
         b = Male2contrast,
         response = Male1attention/Totalattention) %>%
...
write.csv(data, "../../results/standard_data/kwek2023_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     response=data$response, weights=data$N,
                     family=gaussian(link="identity"),
                     response_lab="discrimination performance",
                     stimulus="male contrast",
                     ref="Kwek et al. 2023",
                     file_prefix="kwek2023",
                     n_pairs=n_pairs)
