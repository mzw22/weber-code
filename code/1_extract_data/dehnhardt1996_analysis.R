...
# Import data
input_data <- read.csv("../../data/dehnhardt1996/dehnhardt_ducker_Fig3A_data.csv")

# Standardise data
data <- input_data %>%
  mutate(a = 1.52, # Standard side length
         b = comparison_side_length,
         response = percent_correct/100,
         N = 48) %>% # N trials from figure 3A caption
...
write.csv(data, "../../results/standard_data/dehnhardt1996_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-10, 12),
