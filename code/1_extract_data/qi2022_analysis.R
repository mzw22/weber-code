
# 1. Setup ---------------------------------------------------------------------

# Set working directory to source file location
setwd(dirname(rstudioapi::getActiveDocumentContext()$path))

# Download my package for Weber's law analysis
#library(devtools)
#install_github("mzw22/kber")

# Load packages: install first using install.packages("tidyverse") etc
library(tidyverse)
library(fmsb) #for calculating Nagelkerke's R^2
library(kber) #for Weber's law analysis

# Load source code
source("../analysis_functions.R") #plots/summaries

# 2. Standardise data ----------------------------------------------------------

# Import data with name from original study code
input_data <- read.csv("../../data/qi2022/qi2022_Fig1D.csv")

# Standardise data
data <- input_data %>%
  pivot_longer(cols=c(4:31), names_to="mouse", values_to="percent_correct") %>%
  filter(!is.na(percent_correct)) %>%
  mutate(a = grit1, b = grit2,
         response = percent_correct/100, #binomial data
         N=1) %>% #NO N (placeholder: N = 1)
  # Get absolute magnitude difference and mean magnitude
  mutate(abs_diff = abs(a - b),
         mean_ab = (a + b)/2) %>% #mean complexity in nest
  # Keep only the relevant columns
  dplyr::select(c(a, b, abs_diff, mean_ab, response, N))
warning("NO N GIVEN (N=1 placeholder)")

# Test plot (k = 1)
plot(response~I(abs_diff/mean_ab), data=data, ylim=c(0, 1))

# Export standardised data
write.csv(data, "../../results/standard_data/qi2022_standard.csv",
          row.names=FALSE)
  
# 3. Estimate k ----------------------------------------------------------------

# Check number of unique magnitude pairs
data <- data %>%
  mutate(pair = ifelse(a < b,
                       paste(a, b, sep="_"),
                       paste(b, a, sep="_")))
n_pairs <- length(unique(data$pair))
n_pairs
# n = 5

# Find best value of k + 95% conf intervals
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-5, 10),
                         response=data$response,
                         family=binomial(link="logit"))
k_estimate
# wonky AIC plot
