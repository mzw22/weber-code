...
# Import data
input_data <- read.csv("../../data/klink2004/klink_klump_fig1_FA.csv")

# Correct for false alarms
data <- input_data %>%
  mutate(hitrate = (HIT - FA)/(100 - FA)) %>%
  mutate(hitrate = ifelse(hitrate > 1, 1, hitrate)) %>% #correct values above 100%
  mutate(hitrate = ifelse(hitrate < 0, 0, hitrate)) #correct values below 0%

# Standardise data
data <- data %>%
  mutate(a = ref_duration,
         b = tone_duration,
         response = hitrate,
         N = 50) %>% #60 trials, first 10 trials discarded
...
write.csv(data, "../../results/standard_data/klink2004_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-0.5, 2),
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="duration",
                     ref="Klink and Klump 2004",
                     file_prefix="klink2004",
                     n_pairs=n_pairs)
