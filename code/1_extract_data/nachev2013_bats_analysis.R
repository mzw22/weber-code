...
# Import data
input_data <- read.csv("../../data/nachev2013/WL_cumulative_data_Bats.csv")

# Remove average values
data <- input_data %>%
  filter(Animal!="Average")
...
write.csv(data, "../../results/standard_data/nachev2013_bats_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="sugar conc.",
                     ref="Nachev et al. 2013 (bats)",
                     file_prefix="nachev2013_bats",
                     n_pairs=n_pairs)
