...
# Import data
input_data <- read.csv("../../data/romo2003/romo2003_Fig1_data.csv")

# Standardise data
data <- input_data %>%
  mutate(a = round(base), #round graphgrabber numbers
         b = round(comparison),
         response = percent_correct/100,
         N = 10) %>% #10 trials per stimulus pair in other papers, idk about this one
...
write.csv(data, "../../results/standard_data/romo2003_standard.csv",
          row.names=FALSE)
