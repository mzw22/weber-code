...
data <- input_data %>%
  ...
  mutate(ConcA = 1/10^as.numeric(ConcIndexA),
         ConcB = 1/10^as.numeric(ConcIndexB))
...
write.csv(data, "../../results/standard_data/iwanir2019_volatiles_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="chemotaxis index",
                     ref="Iwanir et al. 2019 (volatiles)",
                     file_prefix="iwanir2019_volatiles",
                     n_pairs=n_pairs)
