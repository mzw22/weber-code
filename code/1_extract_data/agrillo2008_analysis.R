...
# Import data
input_data <- read.csv("../../data/agrillo2008/agrillo2008_data.csv")

# Add new columns
data <- input_data %>%
  mutate(a = n1, b = n2,
         response = prop_time_larger) %>% #linear variable
...
write.csv(data, "../../results/standard_data/agrillo2008_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab,
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     response=data$response,
                     family=gaussian(link="identity"),
                     response_lab="prop. time with larger shoal",
                     stimulus="number of conspecifics",
                     ref="Agrillo et al. 2008",
                     file_prefix="agrillo2008",
                     n_pairs=n_pairs)
