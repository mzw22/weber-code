...
# Import data
input_data <- read.csv("../../data/chen2013/chen2013_fig2.csv")

# Any other changes go here
data <- input_data %>%
  mutate(p_correct = ifelse(contrast > 30,
                                  p_higher, 1-p_higher)) %>%
  mutate(p_correct = ifelse(p_correct > 1, 1, p_correct)) %>% #correct graphgrabber
  mutate(p_correct = ifelse(p_correct < 0, 0, p_correct)) #correct graphgrabber
# Standardise data
data <- data %>%
  mutate(a = contrast,
         b = 30, #sample
...
write.csv(data, "../../results/standard_data/chen2013_standard.csv",
          row.names=FALSE)
