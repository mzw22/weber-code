...
# Import data (in two parts)
performance <- read.csv("../../data/camarillo2012/fig_1b.csv")
psychophysics_results <- read.csv("../../data/camarillo2012/psychophysics_results.csv")

sample_sizes <- psychophysics_results %>%
  group_by(f1, f2) %>%
  summarise(total_trials = sum(total.of.trials))

# Combine these two parts
input_data <- left_join(performance, sample_sizes)

# Standardise data
data <- input_data %>%
  mutate(a = f1, b = f2,
         response = percentage_correct/100, #binomial data
         N = total_trials) %>%
...
write.csv(data, "../../results/standard_data/camarillo2012_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-0.5, 1),
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="vibrotactile freq.",
                     ref="Camarillo et al. 2012",
                     file_prefix="camarillo2012",
                     n_pairs=n_pairs)
