...
# Import data
input_data <- read.csv("../../data/thuenken2014/Data Thuenken et al 2014.csv")

# Subset: food experiment
data <- input_data %>%
  mutate(a = Shoal_A, b = Shoal_B,
         response = Zeit_A/(Zeit_A+Zeit_B)) %>% #linear data
...
write.csv(data, "../../results/standard_data/thuenken2014_standard.csv",
          row.names=FALSE)
...
k_estimate
#Shouldn't really be included because there are two dips
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     response=data$response,
                     family=gaussian(link="identity"),
                     response_lab="Prop. time spent with larger shoal",
                     stimulus="Shoal size",
                     ref="Thuenken et al. 2014",
                     file_prefix="thuenken2014",
                     n_pairs=n_pairs)
