...
# Import data
input_data <- read.csv("../../data/hauf2008/hauf_fig3.csv") %>%
  filter(training == "relative")

# Standardise data
data <- input_data %>%
  mutate(a = test_stimulus,
         b = 3,
         response = percent_choice/100,
         N = 12) %>% #Fig 3
...
write.csv(data, "../../results/standard_data/hauf2008_rel_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-10, 15),
