...
# Import data
input_data <- read.csv("../../data/bogale2011/bogale_figure3.csv")

# Standardise data
data <- input_data %>%
  mutate(a = n1, b = n2,
         response = prop_chose_larger, #binomial data
         N = 20*8) %>% #20 trials, 8 animals
...
write.csv(data, "../../results/standard_data/bogale2011_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(0, 10),
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="quantity",
                     ref="Bogale et al. 2011",
                     file_prefix="bogale2011",
                     n_pairs=n_pairs)
