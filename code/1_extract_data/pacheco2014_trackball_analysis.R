...
data <- input_data %>%
  ...
  filter(Experiment.Method == "Trackball") # Only treadmill (no arena)
...
write.csv(data, "../../results/standard_data/pacheco2014_trackball_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(0, 15),
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="Call SPL",
                     ref="Pacheco and Bertram 2014 (trackball)",
                     file_prefix="pacheco2014_trackball",
                     n_pairs=n_pairs)
