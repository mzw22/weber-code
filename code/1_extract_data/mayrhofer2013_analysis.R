...
# Import data
input_data <- read.csv("../../data/mayrhofer2013/mayrhofer_fig3.csv")

# Standardise data
data <- input_data %>%
  mutate(a = 90, #reference freq
         b = 90 - freq_difference, #distractor freq
         response = percent_correct/100,
         N = 270) %>% #lower bound from paper
...
write.csv(data, "../../results/standard_data/mayrhofer2013_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(0.5, 3),
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     ref="Mayrhofer et al. 2013",
                     file_prefix="mayrhofer2013",
                     n_pairs=n_pairs)
