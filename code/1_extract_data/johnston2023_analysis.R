...
# Import data
input_data <- read.csv("../../data/johnston2023/testing_data.csv")

# Standardise data
data <- input_data %>%
  mutate(correct_side = ifelse(left_p > right_p, "left", "right")) %>%
  mutate(a = left_p/10,  #convert to probability
         b = right_p/10) %>% #convert to probability
  mutate(choice_side = ifelse(choice == 1, "left",
                              ifelse(choice == 2, "right", NA))) %>%
  mutate(response = ifelse(choice_side == correct_side, 1, 0)) %>% #binary data
  mutate(response = ifelse(is.na(response), 0, response)) %>% #filter out non response
...
write.csv(data, "../../results/standard_data/johnston2023_standard.csv")
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-2, 1),
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="reward probability",
                     ref="Johnston et al. 2023",
                     file_prefix="johnston2023",
                     n_pairs=n_pairs)
