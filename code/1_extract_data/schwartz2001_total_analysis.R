...
data <- input_data %>%
  group_by(trial) %>%
  mutate(total_pulses = pulses_per_call*number_calls) %>%
  mutate(ranked_total_pulses = rank(-total_pulses)) %>%
  mutate(best_male = ifelse(ranked_total_pulses == 1, 1, 0)) %>%
  mutate(correct = ifelse(best_male == male_chosen, 1, 0)) %>%
  mutate(best_total_pulses = mean(ifelse(best_male == 1, total_pulses, NA), na.rm=TRUE)) %>%
  ungroup()

# Standardise data
data <- data %>%
  mutate(a = total_pulses,
         b = best_total_pulses,
...
write.csv(data, "../../results/standard_data/schwartz2001_total_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     ref="Schwartz et al. 2001 (total)",
                     file_prefix="schwartz2001_total",
                     n_pairs=n_pairs)
