...
# Import data
exp1 <- read.csv("../../data/beran2012/beran_experiment3.csv")
exp3 <- read.csv("../../data/beran2012/beran_experiment1.csv")
input_data <- rbind(exp1, exp3)

# Standardise data
data <- input_data %>%
  mutate(a = Left, b = Right,
         response = Choice) %>% #binary data
...
write.csv(data, "../../results/standard_data/beran2012_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="food quantity",
                     ref="Beran 2012",
                     file_prefix="beran2012",
                     n_pairs=n_pairs)
