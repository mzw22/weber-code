...
# Import data
input_data <- read.csv("../../data/talwar2001/talwar_and_gerstein_2001_data.csv")

# Standardise data
data <- input_data %>%
  mutate(a = F_plus_deltaF,
         b = Test_Frequency_kHz,
         response = Performance, #binomial data
         N = N_trials) %>% #sample size
...
write.csv(data, "../../results/standard_data/talwar2001_standard.csv",
          row.names=FALSE)
