...
# Import data
input_data <- read.csv("../../data/liu2005/liu_figure2.csv")

# Standardise data
data <- input_data %>%
  mutate(a = 16, #reference speed
         b = 16+16*(percent_speed_difference/100),
         response = ifelse(b > a,
                           percent_faster/100,
                           (100 - percent_faster)/100),
         N = 20) %>% #lower bound for no. repeats for each trial type in a block (20-30)
...
write.csv(data, "../../results/standard_data/liu2005_standard.csv",
          row.names=FALSE)
