...
# Load source code
source("../analysis_functions.R") #plots/summaries (requires k_functions.R)

# 2. Standardise data ----------------------------------------------------------

# Import data
input_data <- read.csv("../../data/baker2011/baker2011_fig1.csv")

# Standardise data
data <- input_data %>%
  mutate(a = ratio*433.33, #433.33 = total mass of food
         b = (1-ratio)*433.33,
         response = prop_larger, #binomial data
         N = 16) %>% #n coyotes (from paper)
...
write.csv(data, "../../results/standard_data/baker2011_standard.csv",
          row.names=FALSE)
...
# Can't fit: mean is the same in every trial
