...
# Standardise data
data <- input_data %>%
  mutate(a = as.numeric(calls["Pulse Interval", treatment_col]),
         b = as.numeric(calls["Pulse Interval", "X0_Control"]),
...
write.csv(data, "../../results/standard_data/tumulty2022_interval_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-20, 10),
                         response=data$response, family=gaussian(link="identity"))
k_estimate
