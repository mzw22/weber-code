...
# Import data
input_data <- read.csv("../../data/vonstebut2001/vonstebut_schmidt_fig_2.csv")

# Add extra info to data
data <- input_data %>%
  # Add column for test freq
  mutate(ft = ft_minus_fr + fr) %>% #test freq
  # Determine if choice was correct or not
  mutate(correct = ifelse(ft > fr, ft_higher, 100 - ft_higher))
...
data <- data %>%
  mutate(a = ft,
         b = fr,
         response = correct/100, #binomial data
         N = 30) %>% #approx n (from email correspondence)
...
write.csv(data, "../../results/standard_data/constebut2001_standard.csv",
          row.names=FALSE)
...
k_estimate <- estimate_k(di=data$abs_diff, i=data$mean_ab, k_range=c(-5, 5),
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="sound freq.",
                     ref="von Stebut and Schmidt 2001",
                     file_prefix="vonstebut2001",
                     n_pairs=n_pairs)
