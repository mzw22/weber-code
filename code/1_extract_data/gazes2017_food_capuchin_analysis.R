...
data <- input_data %>%
  filter(Stimuli == "Food", Species == "Capuchin") %>%
...
write.csv(data, "../../results/standard_data/gazes2017_food_capuchin_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     ref="Gazes et al. 2017 (food, capuchin)",
                     file_prefix="gazes2017_food_capuchin",
                     n_pairs=n_pairs)
