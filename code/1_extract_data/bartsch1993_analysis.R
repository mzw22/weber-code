...
# Import data
input_data <- read.csv("../../data/bartsch1993/bartsch1993_fig2.csv")

#Standardise data
data <- input_data %>%
  mutate(a = standard + delta_ft,
         b = standard,
         response = percent_correct/100,
         N = 30) %>% # Approximate N - from email correspondence
...
# Considered removing extreme outlier but decided against messing with the data
#standard_data <- standard_data %>%
#filter(response > 0.2)

# Export standard data
write.csv(data, "../../results/standard_data/bartsch1993_standard.csv",
          row.names=FALSE)
...
# Import data
data <- read.csv("../../results/standard_data/bartsch1993_standard.csv")
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="frequency",
                     ref="Bartsch and Schmidt 1993",
                     file_prefix="bartsch1993",
                     n_pairs=n_pairs)
