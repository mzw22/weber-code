...
data <- input_data %>%
  filter(Stimuli == "Pebbles", Species == "Capuchin") %>%
...
write.csv(data, "../../results/standard_data/gazes2017_pebbles_capuchin_standard.csv",
          row.names=FALSE)
...
export_model_results(di=data$abs_diff, i=data$mean_ab, k_estimate=k_estimate,
                     ...
                     stimulus="Quantity (pebbles)",
                     ref="Gazes et al. 2017 (pebbles, capuchin)",
                     file_prefix="gazes2017_pebbles_capuchin",
                     n_pairs=n_pairs)
