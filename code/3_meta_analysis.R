...
k_results_table <- k_results_full %>%
  mutate(k_95_intervals = paste(format(round(k_lower_95, 2), nsmall=2), ", ",
                                format(round(k_upper_95, 2), nsmall=2),
                                sep = "")) %>% #column for formatted k estimate
  # Get reasons for exclusion
  mutate(exclude = ifelse(interval > 3, "confidence interval > 3", exclude)) %>%
  arrange(exclude) %>%
  mutate(k_estimate = round(k_estimate, 2)) %>% #round some values
  mutate(test.statistic = round(test.statistic, 2)) %>%
  # Calculate AIC differences
  # Format table
  mutate(k_p_stars = paste(ifelse(p.value < 0.001, "<.001", round(p.value, 4)),
                           stars.pval(p.value), sep="")) %>%
  mutate(model_fit = paste(test.statistic, " (p = ", k_p_stars, ")", sep="")) %>%
  mutate(k_full = paste(k_estimate, " [", k_95_intervals, "]", sep="")) %>%
  dplyr::select(c("reference_lab", "tree_label", "stimulus",
                  "modality", "preference", "task", "weber_reported",
                  "k_full", "model", "model_fit", "exclude"))
colnames(k_results_table) <- c("Reference", "Species", "Stimulus",
                               "Modality", "Choice", "Task", "Weber's law",
                               "k", "Model fitted", "Model test statistic",
                               "Exclusion reason")
write.csv(k_results_table, "../results/meta_analysis/k_results_table.csv", na="", row.names=FALSE)
...
phylo_brlen <- compute.brlen(phylogenetic_tree)
...
forest_plot <- ggplot(k_results, aes(y=reference_lab, x=k_estimate))+
  ...
  geom_vline(xintercept=1, colour="black", linetype="solid", linewidth=0.4)+
...
mods_vif <- vif(model_mods, table=TRUE)
mods_vif
...
mods_table <- data.frame(moderator=rownames(model_mods$b),
                         ...
                         vif = c(NA, round(mods_vif$vifs, 2)))
mods_table
# Export table
...
mean_plot_template <- ggplot(mapping=aes(x=k_mean, y=levels,
  ...
  # Reference lines
  geom_vline(xintercept=1, col="black", linetype="solid", linewidth=0.4)+ #k=0
  geom_vline(xintercept=0, col="black", linetype="dashed", linewidth=0.4)+ #k=1
  #axis limits
...
weber_plot <- mean_plot_template+
  ...
  geom_errorbarh(data=filter(k_means, mod=="reported"),
                 height=0, linewidth=0.7)+
  labs(title="Weber's law reported")
weber_plot
...
# Remove the outlier to see if significance disappears
k_results_late <- k_results %>%
  filter(source != "late") #added_late is blank

model_random_late <- rma.mv(yi=k_estimate, V=I(standard_error^2),
                     random=list(~1|reference, ~1|species, ~1|phylogeny),
                     R=list(phylogeny=phylo_corr_matrix),
                     data=k_results_late, method="REML")
model_random_late
# not affected

model_mods_late <- rma.mv(yi=k_estimate, V=I(standard_error^2),
                          mods=~preference+task+modality+weber_reported,
                          random=list(~1|reference, ~1|species, ~1|phylogeny),
                          R=list(phylogeny=phylo_corr_matrix),
                          data=k_results_late, method="REML")
...
tree_plot_names <- ggplot(k_species_rel, aes(y=tree_label))+
  ...
  theme(panel.background = element_blank(),
        ...
        plot.margin = unit(c(0.6, 0.1, 0.3, 0.1), units="cm"))
tree_plot_names

tree_species_plot <- grid.arrange(tree_cropped, tree_plot_names,
                                  ncol=2, widths=c(1.5, 3))
...
# Ppt forest plot
forest_plot_ppt <- forest_plot+
  theme(axis.text.y = element_blank(),
        axis.text.x = element_text(size=14),
        axis.title.x = element_text(size=14),
        panel.border = element_blank(),
        axis.line.x = element_line(colour = "black"))+
  scale_x_continuous(breaks=c(0, seq(-2, 10, 1)))+ #set x axis
  scale_y_discrete(breaks=NULL)
forest_plot_ppt

ggsave(forest_plot_ppt, file="../results/figures/ppt/forest_plot.png",
       width=4, height=5, dpi=300)

# Without mean
forest_plot_ppt_2 <- forest_plot_ppt
forest_plot_ppt_2$layers[[1]] <- NULL #remove line
forest_plot_ppt_2
...
# Ppt phylogeny
tree_plot_ppt <- tree_plot+
  theme(legend.title = element_text(size=14),
        legend.text = element_text(size=14))
tree_plot_ppt

ggsave(tree_plot_ppt, file="../results/figures/ppt/tree_plot.png",
       width=3, height=4.5, dpi=300)

# Ppt funnel plot
funnel_plot_ppt <- ggplot(k_results, aes(x=k_estimate, y=standard_error)) +
  labs(y="standard error", x="k", shape="Weber's law")+
  #Add confidence intervals
  geom_line(aes(x=lower_95, y=se), data=funnel_lines,
            linetype="dashed", col="black", linewidth=0.5)+ #left line
  geom_line(aes(x=upper_95, y=se), data=funnel_lines,
            linetype="dashed", col="black", linewidth=0.5)+ #right line
  #stat_function(fun=se_fun, geom="area", n=1000,
  #fill="darkgrey", col=NA, alpha=0.5)+ #area
  # Add estimate as a vertical line
  geom_vline(aes(xintercept=estimate),
             col="red", linetype="solid", linewidth=0.5)+
  # Add data
  geom_point(size=1)+
  #geom_text_repel(aes(label=ID), size=4, nudge_y=0.02,
  #box.padding=0.01, max.overlaps=12)+
...
ggsave(funnel_plot_ppt, file="../results/figures/ppt/funnel_plot.png",
...
ggsave(results_detail, file="../results/figures/results_detail.png",
       width=150, height=230, unit="mm", dpi=300)
